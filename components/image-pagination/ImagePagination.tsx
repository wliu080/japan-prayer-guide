...
const ImagePagination = ({
  ...
  const onNextClick = useCallback((e: React.MouseEvent) => {
    e.preventDefault();
  ...
  const onPrevClick = useCallback((e: React.MouseEvent) => {
    e.preventDefault();
    if (activeIndex !== 0) {
      setActiveIndex(activeIndex => activeIndex - 1);
    }
  ...
  return (
    <>
      <Container className='d-flex justify-content-center'>
        <button
          onClick={onPrevClick}
          className='image-pagination-button'
        >
          <RiArrowLeftSLine size={32} />
        </button>
        <Container
          className='image-pagination-images'
      ...
      {
        barDisplay && <div className='d-flex text-center justify-content-center'>
          {pages.map((img, idx) => (
            <div
              key={`${img.src}_${idx}`}
